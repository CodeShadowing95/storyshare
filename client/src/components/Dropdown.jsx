...
const Dropdown = ({ post, user, onToggleModal }) => {
  const navigate = useNavigate();

  const editPost = (post) => {
    navigate(`/edit-post?post=${post._id}`)
  }

  return (
    <div className="absolute translate-y-2 -translate-x-[82%] z-10">
      <div className="w-[250px] py-1 bg-white rounded-md border relative">
        {/* If the user is the creator of the post */}
        {post.creator === user.username && (
          <>
          <div className="w-full flex items-center px-4 py-2 hover:bg-gray-200 cursor-pointer" onClick={() => editPost(post)}>
          ...
          <div className="w-full flex items-center px-4 py-2 hover:bg-gray-200 cursor-pointer" onClick={() => onToggleModal(post._id)}>
            <div className="text-sm inline-flex gap-2 items-center font-semibold">
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24"><path fill="#ef4444" d="M7 21q-.825 0-1.412-.587T5 19V6H4V4h5V3h6v1h5v2h-1v13q0 .825-.587 1.413T17 21zm2-4h2V8H9zm4 0h2V8h-2z"/></svg>
              <p className="text-[13px] text-red-500">Supprimer mon post</p>
            </div>
          </div>
          </>
        )}
        ...
        {post.creator !== user.username && (
          <>
          <div className="w-full flex items-center px-4 py-2 hover:bg-gray-200 cursor-pointer" onClick={() => alert("Invitation envoyée")}>
            <div className="text-sm inline-flex gap-2 items-center font-semibold">
              ...
              <p className="text-[13px] font-medium">Envoyer une invitation</p>
            </div>
          </div>
          <div className="w-full h-[1px] bg-gray-200" />
          <div className="w-full flex items-center px-4 py-2 hover:bg-gray-200 cursor-pointer">
            <div className="text-sm inline-flex gap-2 items-center font-semibold">
              ...
              <p className="text-[13px] font-medium">Masquer tous ses posts</p>
            </div>
          </div>
          <div className="w-full h-[1px] bg-gray-200" />
          <div className="w-full flex items-center px-4 py-2 hover:bg-gray-200 cursor-pointer">
            <div className="text-sm inline-flex gap-2 items-center font-semibold">
              ...
              <p className="text-[13px] font-medium">Signaler le post</p>
            </div>
          </div>
          </>
        )}

        <div className="absolute w-4 h-4 top-0 right-4 -rotate-45 bg-white border-t border-r -translate-y-1/2"></div>
      </div>
    </div>
  )
}
